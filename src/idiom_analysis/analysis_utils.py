...
class IdiomUtils():
    ...
    def calculate_ensemble_measures(prediction_files, truth_files):
        ...
        for i in range(len(prediction_list[0])):
            pred_cluster_i = IdiomUtils.group_ensemble_data(prediction_list, i)
            measures = IdiomUtils.compute_cluster_measures(pred_cluster_i)
            tightness.append(measures["tightness"])
            radii.append(measures["radius_mean"])
            densities.append(measures["density"])

            error_cluster_i = IdiomUtils.group_ensemble_data(absolute_errors, i)
            error_measures = IdiomUtils.compute_cluster_measures(error_cluster_i)
    ...
    def describe_cluster(df, cluster_idx, normalize=False, ignore_columns=None):
        ...
        if normalize:
            df_to_describe = IdiomUtils.quantile_normalize(df, cols_to_use)
        else:
            df_to_describe = df
    ...
    def identify_significant_peaks_by_range(df, reference_df, measure, sigma=1):
        df["surrounding_seq_idx"] = df.index.map(lambda idx: IdiomUtils.get_surrounding_peak_sequence(idx, reference_df, context_size=4))
    ...
    def identify_significant_peaks_by_prominence(df, reference_df, measure, sigma=1):
        ...
        df["smoothed_uncertainty"] = df.apply(lambda row: IdiomUtils.get_smoothed_uncertainty_sequence(reference_df, row.name, measure, sigma), axis=1)
        ...
        df["height"] = df.apply(lambda row: IdiomUtils.calculate_height(df, row.name), axis=1)
    ...
    def compare_idiom_similarity_by_volume(idiom_sequences, ground_truth, sample_size, seq_length):
        ...
        idiom_df["chirp_attributes"] = idiom_df["chirp_indices"].map(
            lambda indices: IdiomUtils.get_attributes_from_indices(indices, ground_truth)
        )
        idiom_df["length"] = idiom_df["chirp_indices"].map(lambda indices: len(indices))

        idiom_volumes = []
        for _ in range(sample_size):
            seq = IdiomUtils.get_random_idiom_sequence(seq_length, idiom_df)
            attributes = IdiomUtils.get_attributes_from_indices(seq, ground_truth)
            volume = IdiomUtils.calculate_sequence_volume(attributes)
        ...
        non_idiom_volumes = []
        for _ in range(sample_size):
            seq = IdiomUtils.get_random_sequence(seq_length, idiom_sequences, ground_truth)
            attributes = IdiomUtils.get_attributes_from_indices(seq, ground_truth)
            volume = IdiomUtils.calculate_sequence_volume(attributes)
    ...
    def compare_idiom_similarity_by_pairwise_distance(idiom_sequences, ground_truth, sample_size, seq_length):
        ...
        for i in range(seq_length - 1):
            seq = IdiomUtils.get_random_sequence(1, idiom_sequences, ground_truth)
            non_idiom_indices.append(seq[0])

        non_idiom_attributes = IdiomUtils.get_attributes_from_indices(non_idiom_indices, ground_truth)
