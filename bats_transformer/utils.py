...
def spacetimeformer_predict_calculate_loss(
        ...
        if len(normalized_preds.shape) == 4: # quantile regression
            normalized_preds = normalized_preds[:, :, :, normalized_preds.shape[-1] // 2].view(normalized_preds.shape[0], normalized_preds.shape[1], -1)  # select median quantile          
...
def spacetimeformer_predict_quantized_prob(
        model,
        x_c: torch.Tensor,
        y_c: torch.Tensor,
        x_t: torch.Tensor,
        ...
        with torch.no_grad():
            # gradient-free prediction
            normalized_preds, predicted_probs, *_ = model.forward(
                x_c, y_c, x_t, yhat_t, **model.eval_step_forward_kwargs
            )

        
        return predicted_probs.to(og_device).float()